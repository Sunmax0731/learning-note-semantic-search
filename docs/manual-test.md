...
- 作業ディレクトリ: `D:\AI\VSCodeExtension\learning-note-semantic-search`
...
```powershell
cd D:\AI\VSCodeExtension\learning-note-semantic-search
npm test
npm start
```
...
5. VS Code extension + local note index の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/learning-note-semantic-search-docs.zip` に存在する。
